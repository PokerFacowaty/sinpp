...
class TestRemoveRole(TestCase):
    ...
    def test_remove_role_not_get_or_post(self):
        request = self.factory.delete(f"/remove_role/{self.rl.id}/")
        request.user = self.staff_user
        response = remove_role(request, self.rl.id)
        self.assertEqual(response.status_code, 400)
...
class TestRemoveStaff(TestCase):
    ...
    def test_remove_staff_nonexistent_event(self):
        request = self.factory.delete("/remove_staff/11111/",
                                      data={"payload":
                                            {"username": "betterManager"}},
                                      headers={"X-Requested-With":
                                               "XMLHttpRequest"},
                                      content_type="application/json")
        request.user = self.staff_user
        response = remove_staff(request, 11111)
        self.assertEqual(response.status_code, 404)

    def test_remove_staff_nonexistent_user(self):
        request = self.factory.delete(f"/remove_staff/{self.ev.id}/",
                                      data={"payload":
